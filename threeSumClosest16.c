/* Leetcode - 16. 3Sum Closet - Medium */
...
int main(void)
{
    ...
    printf("Leetcode - 15. 3Sum Closet - Medium\n");

    int nums[10][50] = {{-1,2,1,-4}, {0,0,0}};
    int sizeNumsTest[10] = {4, 3};

    int targetTest[10] = {1,1};
    ...
    for (int test = 0; test < NUMBER_OF_TESTS; test++)
    {
        ...
        const int copyTarget = target;

        bool flagFinder = true;
            ...
            for (int i = 0; i < sizeNums; i++)
            {
                for (int j = 0; j < sizeNums; j++)
                {
                    for (int k = 0; k < sizeNums; k++)
                    {
                        if (i != j && j != k && i != k)
                        {
                            if (nums[test][i] + nums[test][j] + nums[test][k] == target)
                            {
